defmodule DailyployWeb.TagController do
  ...
  def show(conn, _) do
    tag =
      TagModel.get_tag_in_workspace!(%{
        workspace_id: conn.assigns.workspace.id,
        tag_id: conn.assigns.tag.id
      })
  ...
  defp load_tag_by_id_in_workspace(
         %{params: %{"workspace_id" => workspace_id, "id" => tag_id}} = conn,
         _
       ) do
    case TagModel.get_tag_in_workspace!(%{workspace_id: workspace_id, tag_id: tag_id}, [
           :workspace
         ]) do
      %Tag{} = tag ->
        assign(conn, :tag, tag)
