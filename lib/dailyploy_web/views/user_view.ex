defmodule DailyployWeb.UserView do
  ...
  def render("user_auth.json", %{user: user}) do
    %{id: user.id, name: user.name, email: user.email, access_token: user.access_token}
  end
  ...
  def render("user_tasks.json", %{user: user}) do
    %{
      ...
      name: user.name,
      email: user.email,
      date_formatted_tasks: render_many(user.tasks, TaskView, "date_formatted_user_tasks.json")
    }
  end
