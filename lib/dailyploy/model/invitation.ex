defmodule Dailyploy.Model.Invitation do
    ...
    def fetch_token_details(token) do
      query = 
        ...
        %Invitation{name: name, working_hours: working_hours, role_id: role_id, workspace_id: workspace_id, project_id: project_id, email: email} = List.first(Repo.all(query))
        token_details =  %{"name" => name, "email"=> email, "working_hours" => working_hours, "role_id" => role_id, "project_id" => project_id, "workspace_id" => workspace_id}
        token_details
    end
